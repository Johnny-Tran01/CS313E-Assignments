...
def greedy(grid):
    total = grid[0][0]
    j = 0
    # checks to see if the position is greater than the current position. If
    # it is then we can take the value of the new position.
...
def divide_conquer_helper(grid, row, col):
    ...
    if row == len(grid) - 1:
        return grid[row][col]
    # continues to go down the triangle but adds the first and the max
    # together every time the recursion is called.
    else:
...
def dynamic_prog(grid):
    # begins at the second to last tow and the second for loop will go
    # through all col and add the max to whichever is larger.
    for i in range(len(grid) - 2, -1, -1):
        for j in range(i + 1):
            total = grid[i][j]
            total += max(grid[i + 1][j], grid[i + 1][j + 1])
